...
//Serveur Musee Civilisation Quebec
//        Musee Historique Chevalier
//09-10-2013
import java.io.*;
...
void loop()
{
  while (myPort.available () > 0) {
    ...
    if (value.equals("P1")) {
      //launch script
      int val = runAppleScript("StationMCQ.scpt");//Change script name according to station
    } else if (value.equals("P2")) {
      println("P2");
      //launch script
      int val = runAppleScript("StationMHC.scpt");//Change script name according to station
    } else {
      println("WRONG");
    }
